...
import { buildUserSafeCategories } from '@/lib/utils/category';
...
export default async function ProductsPage({ searchParams }: PageProps) {
  ...
  const safeCategories = buildUserSafeCategories(categories, categoryCounts);
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 space-y-6">
          <header className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h1 className="text-5xl font-black tracking-tight text-foreground">All Products</h1>
            ...
            <div className="flex items-center gap-2">
              <ProductsToolbar categories={safeCategories} />
            </div>
          </header>
          ...
          {products.length > 0 ? (
            ...
            <div className="rounded-3xl border border-dashed border-border bg-muted/20 px-6 py-20 text-center">
              <div className="mx-auto mb-4 h-14 w-14 rounded-full bg-background text-2xl leading-[56px] text-muted-foreground">☹</div>
              <h2 className="text-4xl font-black text-foreground">No products found</h2>
